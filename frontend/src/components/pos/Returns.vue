				...
				<v-container>
					...
					>
						...
						<template v-slot:item.status="{ item }">
							<v-chip v-if="!item.can_return" size="small" color="error">
								{{ __("Fully Returned") }}
							</v-chip>
							<v-chip
							...
							<v-alert type="info" density="compact" variant="tonal">
								<div class="d-flex justify-space-between align-center">
									...
									<span
										...
										{{ currencySymbol(selectedInvoice?.currency) }}
										{{ formatCurrency(totalReturnAmount) }}</span
									>
								</div>
							</v-alert>
						</v-col>
...
export default {
			...
			handler(newVal, oldVal) {
				...
				newVal.forEach((itemId) => {
					if (!(itemId in this.returnQuantities)) {
						const item = this.returnableItems.find(
							(i) => i.sales_invoice_item === itemId
						);
						if (item) {
	...
	methods: {
		getReturnableItemStatus(itemName) {
			return (
				this.returnableItems.find((item) => item.sales_invoice_item === itemName) || null
			);
		},
		getIncompleteOfferReturnIssues() {
			...
			invoiceOffers.forEach((offer) => {
				...
				const hasSelectedTrigger = triggeringItems.some((item) =>
		...
		normalizeReturnQty(item, inputValue, showMessage = false) {
			...
			if (qty > item.remaining_qty) {
				...
				if (showMessage) {
					toast.warning(
						__("Cannot return more than {0} {1}", [item.remaining_qty, item.uom])
					);
				}
			}
		...
		submit_return() {
			...
			this.selectedItems.forEach((itemId) => {
				const item = this.returnableItems.find((i) => i.sales_invoice_item === itemId);
				if (item) {
					const return_qty = this.normalizeReturnQty(
						item,
						this.returnQuantities[itemId],
						true
					);
