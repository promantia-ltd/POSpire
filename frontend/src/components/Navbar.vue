<template>
	<nav>
		<v-app-bar height="72" class="modern-header" elevation="1">
					...
					<div class="pospire-brand-section">
						...
						<div class="pospire-logo-animated">
							...
							<span class="pospire-name">
								<span class="pospire-pos">POS</span
								><span class="pospire-pire">pire</span>
							</span>
						</div>
					</div>
				</div>
			...
			<div class="user-info">
				<v-chip
					class="user-chip pospire-chip-neutral"
					variant="tonal"
					color="grey-darken-2"
				>
...
export default {
	...
	methods: {
		...
		go_about() {
			const win = window.open("https://github.com/promantia-ltd/POSpire", "_blank");
			win.focus();
		},
		close_shift_dialog() {
		...
		async print_last_invoice() {
			...
			try {
				const res = await this.hardwareConfiguration(this.pos_profile.name);
