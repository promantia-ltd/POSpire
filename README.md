...
| Layer | Technology |
|-------|------------|
| Backend | Python 3.14, Frappe Framework v16 |
| Frontend | Vue.js 3, Vuetify 3, Vite |
| Database | MariaDB (via Frappe) |
| Build Tools | Vite, ESLint, Ruff |
...
```
pospire/
├── frontend/                   # Vue SPA (primary development target)
│   ├── src/
│   │   ├── components/
│   │   │   ├── pos/            # POS components (Invoice, Items, Payments, Returns…)
│   │   │   ├── payments/       # Payment reconciliation
│   │   │   └── Navbar.vue
│   │   └── utils/              # Shared helpers (datetime, format, call…)
│   ├── eslint.config.js
│   └── vite.config.js
├── pospire/
...
│   │   └── frontend/           # Vite build output (gitignored, generated by bench build)
│   └── hooks.py                # Frappe hooks configuration
├── pyproject.toml              # Python metadata + bench asset build config
└── README.md
```
...
# JavaScript/Vue linting (requires Node 24+)
cd frontend && node_modules/.bin/eslint src --quiet
...
# Production build — outputs to pospire/public/frontend/ and updates pospire/www/pospire.html
bench build --app pospire
```
