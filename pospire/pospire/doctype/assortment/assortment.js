	...
	frappe.call({
		...
		callback: function (r) {
			...
			items.forEach((item) => {
				...
				let safe_name = frappe.utils.escape_html(item.name);
				let safe_item_name = frappe.utils.escape_html(item.item_name);
					...
					<div class="item-row" style="padding:6px; border-bottom:1px solid #f0f0f0;">
						<label style="cursor:pointer; display:flex; gap:10px;">
							<input
								type="checkbox"
								value="${safe_name}"
								data-item_name="${safe_item_name}"
								${checked}
							/>
							<div>
								<div style="font-weight:600;">${safe_item_name}</div>
								<div style="font-size:12px; color:gray;">${safe_name}</div>
							</div>
						</label>
					</div>
				`;
			...
			d.$wrapper.find("#item-search").on("input", function () {
