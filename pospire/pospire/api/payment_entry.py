...
from pospire.pospire.api.m_pesa import submit_mpesa_payment
		...
		if include_paid == "false":
			...
			for invoice in outstanding_invoices:
				if invoice.get("currency") == currency:
					if (
						pos_profile_name
						and frappe.get_cached_value("Sales Invoice", invoice.get("voucher_no"), "pos_profile")
						!= pos_profile_name
					):
						continue
			...
			filters = {"company": company, "outstanding_amount": ("=", 0), "is_return": 0}
			if customer:
...
def process_pos_payment(payload: str) -> dict:
	...
	_pos_opening_shift_name = data.pos_opening_shift_name  # Reserved for future use
